...
async def ni43101(payload: dict) -> dict:
    result = generate_ni43101_report(
        payload.get("project_name", "Matuu Project"), payload
    )
    audit = build_audit_event(
        {
            "action": "generate_ni43101",
...
async def kenya_el(payload: dict) -> dict:
    result = generate_kenya_el_report(
        payload.get("project_name", "Matuu Project"), payload
    )
    audit = build_audit_event(
        {
            "action": "generate_kenya_el",
            "resource_type": "report",
            "project": payload.get("project_name"),
        }
    )
